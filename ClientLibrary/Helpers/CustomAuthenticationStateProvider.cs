...
{    
    public class CustomAuthenticationStateProvider(LocalStorageService localStorageService) : AuthenticationStateProvider
    {
        ...
        private static CustomUserClaims DecryptToken(string jstToken)
        {
            if (string.IsNullOrEmpty(jstToken)) return new CustomUserClaims();

            var handler = new JwtSecurityTokenHandler();
            var token = handler.ReadJwtToken(jstToken);
